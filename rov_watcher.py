...
PYTHON_EXE      = sys.executable  # Uses current Python interpreter
...
def wait_for_approval(prompt_msg, timeout_sec=600):
        ...
        try:
            ...
            for upd in r.json().get("result", []):
                ...
                if text in YES_WORDS:
                    log.info("Reply: YES")
                    send_telegram("�o. *Approved!* Starting now...")
                    return True
                if text in NO_WORDS:
                    log.info("Reply: NO")
                    send_telegram("�?,�,? *Cancelled.* I will not proceed.")
                    return False
        except Exception as e:
    ...
    send_telegram(
        f"�?� *No reply after {timeout_sec//60} min �?" cancelled.*\n"
...
def process_video(video_path):
    ...
    if size_mb < MIN_FILE_SIZE_MB:
        log.warning(f"Too small ({size_mb:.1f}MB) �?" skipping.")
        return
    ...
    # �"?�"? All clear �?" just notify, no approval needed �"?�"?�"?
    if flagged_count == 0:
        send_telegram(
            f"dY� *ROV Watcher �?" Extraction Complete*\n"
            f"dY"1 `{os.path.basename(video_path)}`\n\n"
            f"dY-��,? Frames: *{total_frames}*\n"
            f"�o. All clear �?" no interesting frames flagged.\n"
            f"No AI analysis needed."
        )
        return

    # �"?�"? Ask approval to run analyzer �"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?
    flag_types = set()
    ...
    approved = wait_for_approval(
        ...
        f"�o. Clear frames:   *{clear_count}*\n"
        f"dY'� API savings:    *~{savings_pct}%*\n\n"
        f"dY"? CV signals: {', '.join(sorted(flag_types))}\n"
        f"dY"? Job: `{job_id}`\n\n"
        f"�?3 *Reply YES to start Gemini analysis*\n"
        f"Reply NO to skip.\n"
...
def main():
    log.info("ROV Watcher v5 �?" Run Once Mode")
    ...
    if len(sys.argv) >= 2:
        ...
        log.info("No video specified �?" finding latest in watch folder...")
